...
from app.services.auth import create_access_token, create_refresh_token, decode_token, hash_password, hash_token_id, verify_password
...
def issue_token_pair(session: Session, user_id: UUID) -> TokenPairResponse:
    ...
    session.add(RefreshToken(user_id=user_id, token_hash=hash_token_id(token_id), expires_at=expires_at))
...
def refresh(payload: RefreshRequest, session: DbSession) -> TokenPairResponse:
    ...
    stored = session.scalar(select(RefreshToken).where(RefreshToken.token_hash == hash_token_id(claims["jti"])))
    if stored is None or stored.user_id != user_id or stored.revoked_at or stored.expires_at <= datetime.now(UTC):
